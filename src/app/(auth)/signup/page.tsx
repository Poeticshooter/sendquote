    ...
    <div className="flex min-h-screen items-center justify-center px-4 bg-[#0A0A0A]">
      <Card className="w-full max-w-sm bg-[#141414] border-white/[0.06] text-white">
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-white/40">Start free, no credit card required</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSignup} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="name" className="text-white/70">Business Name</Label>
              <Input id="name" value={businessName} onChange={(e) => setBusinessName(e.target.value)} required className="bg-white/5 border-white/10 text-white placeholder:text-white/30" placeholder="Acme Corp" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="email" className="text-white/70">Work Email</Label>
              <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="bg-white/5 border-white/10 text-white placeholder:text-white/30" placeholder="you@company.com" />
            </div>
            <div className="space-y-2">
            ...
            <Button type="submit" className="w-full bg-[#00D4AA] text-black hover:bg-[#00D4AA]/90 font-semibold" disabled={loading}>{loading ? "Creating..." : "Create Account"}</Button>
          </form>

          <div className="relative my-6"><div className="absolute inset-0 flex items-center"><span className="w-full border-t border-white/[0.06]" /></div><div className="relative flex justify-center text-xs uppercase"><span className="bg-[#141414] px-2 text-white/30">Or</span></div></div>

          <Button variant="outline" className="w-full border-white/10 text-white/70 hover:bg-white/5" onClick={handleGoogle}>
          ...
          <p className="mt-6 text-center text-sm text-white/40">Already have an account? <Link href="/login" className="text-[#00D4AA] hover:underline">Sign in</Link></p>
        </CardContent>
      </Card>
    </div>
  );
