...
export default function AdminUsersPage() {
  ...
  const filtered = search ? users.filter((u) => u.business_name?.toLowerCase().includes(search.toLowerCase()) || (u as any).users?.email?.includes(search)) : users;
                ...
                <tr key={u.id} className="border-b border-white/[0.06] last:border-0">
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-white">{u.business_name || "Unnamed"}</p>
                        <p className="text-xs text-muted-foreground">{u.users?.email || ""}</p>
                      </div>
                    </div>
                  </td>
                  <td className="p-4 text-muted-foreground">{u.business_name || "—"}</td>
                  <td className="p-4">
