          ...
          <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="lg:hidden flex items-center gap-2 text-sm text-slate-500 hover:text-slate-700:text-slate-200 px-3 py-2 rounded-lg hover:bg-slate-100:bg-slate-800 transition-colors"
              >
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="hidden md:flex items-center gap-2 text-sm text-slate-400 hover:text-slate-600:text-slate-300 px-3 py-2 rounded-lg hover:bg-slate-100:bg-slate-800 transition-colors"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <Link href="/settings" className="text-sm text-slate-500 hover:text-slate-700:text-slate-200 p-2 rounded-lg hover:bg-slate-100:bg-slate-800 transition-colors">
              ...
              <button onClick={handleLogout} className="text-sm text-slate-400 hover:text-slate-600:text-slate-300 p-2 rounded-lg hover:bg-slate-100:bg-slate-800 transition-colors">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        ...
        <main className="flex-1 max-w-6xl mx-auto px-6 py-8 space-y-6 animate-fade-in overflow-y-auto">
            ...
            <div className="bg-white rounded-xl p-5 border border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex-1 w-full sm:w-auto">
                <div className="flex items-center justify-between sm:justify-start gap-3 mb-1.5">
                  ...
                  <span className="text-sm text-slate-500">{stats.month_count} / {FREE_PLAN_LIMIT} quotes used</span>
                </div>
                <div className="w-full sm:w-64 bg-slate-100 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full transition-all duration-500 ${stats.month_count >= FREE_PLAN_LIMIT ? "bg-red-400" : "bg-indigo-500"}`}
                    style={{ width: `${Math.min((stats.month_count / FREE_PLAN_LIMIT) * 100, 100)}%` }}
                  />
                </div>
              </div>
              <Link href="/upgrade" className="shrink-0 text-sm font-medium bg-indigo-600 text-white px-5 py-2 rounded-lg hover:bg-indigo-700:bg-indigo-600 transition-all shadow-sm hover:shadow-md active:scale-[0.98]">
          ...
          {referralCount > 0 && (
            <Link href="/settings" className="block">
              <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl p-4 border border-emerald-200 hover:shadow-md transition-all">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-emerald-800">{referralCount} referral{referralCount > 1 ? "s" : ""}</p>
                      <p className="text-xs text-emerald-600">Click to manage your referral link</p>
                    </div>
                  </div>
              ...
              {selectedQuotes.length > 0 && (
                <div className="flex items-center justify-between bg-indigo-50 rounded-lg px-3 py-2 animate-fade-in">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => bulkUpdateStatus("draft")} className="text-xs text-slate-600 hover:text-indigo-700:text-indigo-300 px-2 py-1 rounded hover:bg-white:bg-slate-700">Mark Draft</button>
                    <button onClick={() => bulkUpdateStatus("sent")} className="text-xs text-slate-600 hover:text-indigo-700:text-indigo-300 px-2 py-1 rounded hover:bg-white:bg-slate-700">Mark Sent</button>
                    <button onClick={() => bulkUpdateStatus("accepted")} className="text-xs text-slate-600 hover:text-emerald-700:text-emerald-300 px-2 py-1 rounded hover:bg-white:bg-slate-700">Mark Accepted</button>
                    <button onClick={() => bulkUpdateStatus("archived")} className="text-xs text-slate-600 hover:text-slate-700:text-slate-200 px-2 py-1 rounded hover:bg-white:bg-slate-700">Archive</button>
                    <button onClick={bulkDelete} className="text-xs text-red-600 hover:text-red-700:text-red-300 px-2 py-1 rounded hover:bg-white:bg-slate-700 font-medium">Delete</button>
                  </div>
                </div>
              )}
                    ...
                    <button
                      ...
                      className={`text-xs whitespace-nowrap px-3 py-1.5 rounded-lg font-medium transition-all ${
                        ...
                        (chip === "Overdue" && filter === "sent" && dateRange.to)
                          ...
                          : "text-slate-500 hover:text-slate-700:text-slate-200 hover:bg-slate-100:bg-slate-800"
                      }`}
                    >
                      {chip}
                    </button>
                  ))}
                  <span className="text-xs text-slate-400 ml-auto hidden sm:inline">{quotes.length} total</span>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  <div className="relative flex-1 min-w-[200px]">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-3 py-2 text-sm border border-slate-200 bg-white text-slate-900 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-200:ring-indigo-800 focus:border-indigo-300:border-indigo-600 transition-all placeholder:text-slate-400:text-slate-500"
                    />
                  </div>
                  <select
                    value={clientFilter}
                    onChange={e => setClientFilter(e.target.value)}
                    className="text-sm border border-slate-200 bg-white text-slate-900 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-200:ring-indigo-800"
                  >
                  ...
                  />
                  {hasActiveFilters && (
                    <button onClick={clearFilters} className="text-xs text-slate-500 hover:text-slate-700:text-slate-200 px-2 py-2">Clear</button>
                  )}
                  {quotes.length > 0 && (
                    <>
                      <button onClick={exportCSV} title="Export to CSV"
                        className="shrink-0 text-xs font-medium text-slate-500 hover:text-slate-700:text-slate-200 px-3 py-2 rounded-lg border border-slate-200 hover:bg-slate-50:bg-slate-800 transition-all flex items-center gap-1.5">
                      ...
                      <button onClick={exportAll} title="Full data export (ZIP)"
                        className="shrink-0 text-xs font-medium text-indigo-600 hover:text-indigo-700:text-indigo-300 px-3 py-2 rounded-lg border border-indigo-200 hover:bg-indigo-50:bg-indigo-950/30 transition-all flex items-center gap-1.5">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            {hasMore && (
              <div className="p-4 text-center border-t border-slate-100">
                <button onClick={loadMore} disabled={loadingMore}
                  className="text-sm font-medium text-indigo-600 hover:text-indigo-700:text-indigo-300 px-5 py-2 rounded-lg hover:bg-indigo-50:bg-indigo-950/30 transition-all disabled:opacity-50">
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl p-8 w-full max-w-md mx-4 shadow-2xl border border-slate-200 animate-fade-in-scale">
        <button onClick={onDismiss} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600:text-slate-300 p-1">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-slate-500 min-h-[40px] transition-all">{messages[step]}</p>
        </div>
        ...
        <div className="flex gap-3">
          <button onClick={onDismiss} className="flex-1 py-2.5 rounded-xl text-sm font-medium text-slate-600 bg-slate-100 hover:bg-slate-200:bg-slate-700 transition-colors">
            Maybe Later
          </button>
          <Link href="/upgrade" className="flex-1 py-2.5 rounded-xl text-sm font-medium text-center bg-indigo-600 text-white hover:bg-indigo-700:bg-indigo-600 transition-colors shadow-sm">
            Upgrade — ₹299/mo
          </Link>
        </div>
