    ...
    <Card className="border-white/[0.06]">
      ...
      <CardContent className="space-y-3">
        <p className="text-xs text-muted-foreground">
          Get <span className="text-[#00D4AA] font-medium">1 month free</span> for every friend who joins SendQuote.
        </p>
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" className="border-white/10 text-xs flex-1 h-8" onClick={copyLink}>
            {copied ? <Check className="h-3 w-3 mr-1" /> : <Copy className="h-3 w-3 mr-1" />}
            Copy Referral Link
          </Button>
          <Button variant="outline" size="sm" className="border-white/10 text-xs h-8" onClick={() => {
