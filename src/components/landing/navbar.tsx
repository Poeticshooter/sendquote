    ...
    <header className="sticky top-0 z-50 w-full border-b border-white/[0.06] bg-black/80 backdrop-blur-2xl supports-[backdrop-filter]:bg-black/60">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex items-center gap-3 group">
          <Image src="/icon-192.webp" alt="SendQuote" width={192} height={192} className="h-10 w-10 transition-transform group-hover:scale-105" />
        ...
        <button className="md:hidden rounded-lg p-2 text-white/50 hover:bg-white/5 transition-colors duration-200" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Toggle navigation menu" aria-expanded={mobileOpen}>
      ...
      {mobileOpen && (
        <m.div
          ...
          className="md:hidden border-t border-white/[0.06] bg-black px-4 pb-5 pt-3"
        >
